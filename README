...
*   [btrfs-progs](http://www.kernel.org/pub/linux/kernel/people/kdave/btrfs-progs/): Btrfs filesystem utilities >= v3.18.2
*   [Perl interpreter](https://www.perl.org): Probably already installed on your system
*   [OpenSSH](http://www.openssh.org): If you want to tranfer backups from/to remote locations
...
Download and unpack the newest stable [btrbk source tarball](http://digint.ch/download/btrbk/releases/) and type:

    sudo make install

### Gentoo Linux

Grab the digint portage overlay from: `git://dev.tty0.ch/portage/digint-overlay.git`
...
Please consult the [btrbk(1)](http://digint.ch/btrbk/doc/btrbk.html) man-page provided with this package for a full description of the command line options.

# Configuration File

Before running `btrbk`, you will need to create a configuration file. You might want to take a look at `btrbk.conf.example` provided with this package. For a detailed description, please consult the [btrbk.conf(5)](http://digint.ch/btrbk/doc/btrbk.conf.html) man-page.
...
*   a disk having a btrfs volume mounted as `/mnt/btr_pool`, containing a subvolume `rootfs` for the root filesystem and a subvolume `home` for the user data.
*   a backup disk having a btrfs volume mounted as `/mnt/btr_backup`, containing a subvolume `mylaptop` for the incremental backups.
...
Note that you can run btrbk more than once an hour, e.g. by by calling `sudo btrbk run` from the command line. With this setup, all those extra snapshots will be kept for 18 hours.
...
Then run btrbk, with myhost_sync configured _without any targets_ as follows:
...
I you are using raw _incremental_ backups, please make sure you understand the implications (see [btrbk.conf(5)](http://digint.ch/btrbk/doc/btrbk.conf.html), TARGET TYPES).

# Setting up SSH

Since btrbk needs root access on the remote side, it is _very advisable_ to take all the security precautions you can. Usually backups are generated periodically without user interaction, so it is not possible to protect your ssh key with a password. The steps below will give you hints on how to secure your ssh server for a backup scenario. Note that the btrbk executable is not needed on the remote side, but you will need "/sbin/btrfs" from the btrfs-progs package.

btrbk comes with a shell script "ssh_filter_btrbk.sh", which restricts ssh access to sane calls to the /sbin/btrfs command needed for snapshot creation and send/receive operations (see [ssh_filter_btrbk(1)](http://digint.ch/btrbk/doc/ssh_filter_btrbk.html)). Here is an example on how it can be used with ssh:
...
Please refer to [ssh_filter_btrbk(1)](http://digint.ch/btrbk/doc/ssh_filter_btrbk.html) for a description of the "ssh_filter_btrbk.sh" options, as well as [sshd(8)](http://www.openbsd.org/cgi-bin/man.cgi/OpenBSD-current/man8/sshd.8) for a description of the "authorized_keys" file format.

Also consider setting up ssh access for a user dedicated to btrbk and either set suid root on ssh_filter_btrbk.sh or use the "--sudo" option and configure /etc/sudoers accordingly. For even more security, you can setup a chroot environment in /etc/ssh/sshd_config (see [sshd_config(5)](http://www.openbsd.org/cgi-bin/man.cgi/OpenBSD-current/man5/sshd_config.5)).
...
Make sure to also read the [btrbk FAQ page](faq.html). Help improve it by asking!
...
*   Visit the [btrbk project page on GitHub](http://github.com/digint/btrbk) and use the [issues tracker](http://github.com/digint/btrbk/issues) there.
...
btrbk is free software, available under the [GNU General Public License, Version 3](http://www.gnu.org/licenses/gpl.html).
